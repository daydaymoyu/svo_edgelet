...
*/
    ...
    // reprojtct closekfs mappoint and mappoint_candidate
    ...
    // pose optimization
    // only update the 6DoF frame pose, remove measurements with too large reprojection error
    ...
    // for each point, optmize its position with its all observed frame
...
bool FrameHandlerMono::needNewKf(double scene_depth_mean) {
    ...
    for (auto it = last_kf_->fts_.begin(), it_end = last_kf_->fts_.end(); it != it_end; ++it) {
        ...
        Vector2d px1(new_frame_->w2c((*it)->point->pos_)); // pixel
    ...
    for (auto it = overlap_kfs_.begin(), ite = overlap_kfs_.end(); it != ite; ++it) {
        ...
        if (fabs(relpos.x()) / scene_depth_mean < Config::kfSelectMinDist() && fabs(relpos.y()) / scene_depth_mean < Config::kfSelectMinDist() * 0.8 && fabs(relpos.z()) / scene_depth_mean < Config::kfSelectMinDist() * 1.3)
            return false;
    }
    return true;
}
...
void FrameHandlerMono::setCoreKfs(size_t n_closest) {
    ...
    std::partial_sort(overlap_kfs_.begin(), overlap_kfs_.begin() + n, overlap_kfs_.end(),
                      boost::bind(&pair<FramePtr, size_t>::second, _1) > boost::bind(&pair<FramePtr, size_t>::second, _2));
